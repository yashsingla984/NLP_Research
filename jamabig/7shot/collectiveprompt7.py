...
def write_output_with_index_to_csv(index, output):
    with open('output_data_yash.csv', 'a') as file:
        file.write(f"{index},{output}\n")

# Function to write data to Excel file
def write_to_excel(outputs, extracted_options):
    df = pd.DataFrame({'Output': outputs, 'Extracted Option': extracted_options})
    with pd.ExcelWriter('outputyash123.xlsx', mode='a', if_sheet_exists='replace') as writer:
        df.to_excel(writer, sheet_name='OutputSheet', index=False)
...
#For RY
    ...
    if match==None:
        print("10st one")
        answer_pattern = pattern = re.compile(r"\*\*Answer:\*\*\n([A-D])\. ")
        match = answer_pattern.search(output)
        if match:
            extracted_answer = match.group(1)
            print("Extracted answer:", extracted_answer)
        ...
        if len(fewshot_examples) != 0:
            ...
            for input in tqdm(inputs):
                ...
                try:
                    ...
                    outputs.append(output)
                    extracted_options.append(extracted_option)
                    #write_output_with_index_to_csv(i, output)
                except Exception as e:
                    output="Some_Issues"
                    extracted_option="Some_Issues"
                    outputs.append(output)
